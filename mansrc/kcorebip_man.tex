...
\usepackage[backend=bibtex,style=alphabetic]{biblatex} % (which resembles APA)
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
...
\title{Using the kcorebip package \\ \small{Version 0.9.1}} \author{Javier Garcia-Algarra \\ \href{mailto:jgalgarra@gmail.com}
{jgalgarra@gmail.com} }
...
any kind of bipartite network, but we developed it to study ecological mutualistic communities, so we use terminology and examples of that research field \cite{garcia2018structural,GarciaAlgarra2017}.
...
\item \texttt{graph}: an igraph::graph object. This is the additional information for each node, besides the regular of the object:
	\begin{itemize}
		...
		\item \texttt{V(result\_analysis\$graph)\$krisk}: list $k_{riks}$, a measurement of vulnerability (See \cite{GarciaAlgarra2017})
	\end{itemize}
...
ziggurat_graph(datadir, filename, paintlinks = TRUE,
  ...
  displace_y_b = rep(0, 20), displace_y_a = rep(0, 20), labels_size = 3.5,
  lsize_kcoremax = 3.5, lsize_zig = 3, lsize_kcore1 = 2.5,
  lsize_legend = 4, lsize_core_box = 2.5, labels_color = c(),
...
\item \texttt{paintlinks}: if set to FALSE links will be hidden. It is useful to test the plot appearance when the network is huge.
...
\item \texttt{size\_link}: width of links.
...
\item \texttt{labels\_size}: default node label size.

\item \texttt{lsize\_kcoremax}: nodes in $max\mydash kshell$ label size.

\item \texttt{lsize\_zig nodes}: in inner ziggurats label size.

\item \texttt{lsize\_kcore1}: labels of nodes in $1\mydash shell$.
...
\item \texttt{kcore2tail\_vertical\_separation}: expand vertical of $1\mydash shell$ species linked to $2\mydash shell$ by this factor.

\item \texttt{kcore1tail\_disttocore}: expand vertical separation of $1\mydash shell$ species from $max\mydash shell$ (guild\_a, guild,b).

\item \texttt{innertail\_vertical\_separation}: expand vertical separation of $kshell$ species connected to $2\mydash shell < kshell < max\mydash shell$.

\item \texttt{horiz\_kcoremax\_tails\_expand}: expands horizontal separation of weird tails connected to $max\mydash shell$.

\item \texttt{factor\_hop\_x expand inner}: expands ziggurats horizontal distance.

\item \texttt{displace\_legend modify}: expands legend position by these fractions.

\item \texttt{fattailjumphoriz}: displace  species linked to leftmost $max\mydash shell$ species.
...
\item \texttt{paint\_outsiders}: paint species disconnected of the giant component.
...
\item \texttt{weirdskcore2\_horizontal\_dist\_rootleaf\_expand}: expand horizontal distance of weird tail root node connected to $2\mydash shell$.

\item \texttt{weirdskcore2\_vertical\_dist\_rootleaf\_expand}: expand vertical distance of weird tails connected to  $2\mydash shell$.

\item \texttt{weirds\_boxes\_separation\_count}: weird species boxes separation count.

\item \texttt{root\_weird\_expand}: expand root weird distances of tails connected to $kshell \neq 2$.
...
\item \texttt{kcore1weirds\_leafs\_vertical\_separation}: expand vertical separation of weird tails connected to $1\mydash shell$ species.
...
\item \texttt{weighted\_links}: weight function to display links, (\texttt{"none"},\texttt{"log10"},\texttt{"ln"},\texttt{"sqrt"})

\item \texttt{square\_nodes\_size\_scale}: scale the area of nodes in $1\mydash shell $ or outsiders.
